...
def run(host: str, port: int, distance_threshold_km: float, db_path: str, stale_timeout_seconds: float) -> None:
    notifier = default_notifier()
    storage = TrackStorage(db_path) if db_path.lower() != "none" else None
    if storage is not None:
        logger.info("Logging fixes and events to %s", db_path)
    tracker = FixTracker(notifier, distance_threshold_km=distance_threshold_km, storage=storage)
...
def main() -> None:
    ...
    try:
        run(args.host, args.port, args.distance_threshold_km, args.db, args.stale_timeout_seconds)
